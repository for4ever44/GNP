...
class TDataset(InMemoryDataset):
    ...
    def __init__(self, root, name):
        ...
        # d += int(s.max()) + 1;
        # data = TemporalData(src=s, dst=d, t=t, msg=msg, y=y)
...
class Study:
    ...
    def __init__(self, dataset, ablation):
        with open("./" + dataset + ".txt", 'a+', encoding='utf-8',) as f:
            f.write(f'GNP traning @{dataset} starting ...\n')
            f.close()
        ...
        for epoch in range(50):
            loss = self.train(trn_loader, trn_n_events, pack, criterion)
            if epoch % every_check == 0:
                vap, vauc = self.eval(val_loader, pack)
                print(f'Epoch{epoch+1:02d}, Training Loss:{loss:.4f}, Validation AP:{vap:.4f} & ROC AUC: {vauc:.4f}')
                with open("./" + dataset + ".txt", 'a+', encoding='utf-8', ) as f:
                    f.write(f'Epoch{epoch + 1:02d}, Training Loss:{loss:.4f}, Validation AP:{vap:.4f} & ROC AUC: {vauc:.4f}\n')
                    f.close()
                if vap > best_vap:
                    best_vap = vap
                    stop_count = 0
                    tap, tauc = self.eval(tst_loader, pack);
                    print(f'Test AP: {tap:.4f} & ROC AUC:{tauc:.4f}')
                    with open("./" + dataset + ".txt", 'a+', encoding='utf-8', ) as f:
                        f.write(f'Test AP: {tap:.4f} & ROC AUC:{tauc:.4f}\n')
                        f.close()
                else:
                    stop_count += 1
                if stop_size == stop_count:
                    print('early stop training')
                    with open("./" + dataset + ".txt", 'a+', encoding='utf-8', ) as f:
                        f.write('early stop training')
                        f.close()
                    break
    ...
    def get_dataloaders(self, name):
        ...
        trn_loader, val_loader, tst_loader = [TDataLoader(d, batch_size=200, neg_sampling_ratio=1.0) for d in tvt]
...
if __name__ == '__main__':
    for run in range(5):
        ...
        # [Study(d, 'past') for d in ['mooc', 'lastfm']]

        # [Study(d, 'now') for d in ['mooc', 'lastfm']]
        # [Study(d, 'both') for d in ['wikipedia', 'reddit', 'mooc', 'lastfm']]
        #
